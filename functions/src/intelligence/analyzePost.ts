// functions/src/intelligence/analyzePost.ts
...
export const analyzePost = functions.onDocumentCreated(
  ...
  async (event) => {
    ...
    try {
      ...
      // Get user context from first matching target
...
`;
      ...
      await admin.firestore()
        ...
        .doc('current')
        .update({
          leadsGenerated: admin.firestore.FieldValue.increment(
            analysis.is_lead ? 1 : 0
          )
        });
